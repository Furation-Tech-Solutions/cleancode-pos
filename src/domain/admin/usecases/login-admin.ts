...
export interface LoginAdminUsecase {
  execute: (emailId: string, password: string) => Promise<any>;
}
...
export class LoginAdmin implements LoginAdminUsecase {
  ...
  async execute(emailId: string, password: string): Promise<any> {
    return await this.adminRepository.login(emailId, password);
  }
}
